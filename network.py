...
import numpy as np
from utils import remove_lateral_connections, to_numpy
class DrosophilaOpticLobeCircuit(nn.Module):
    ...
    def __init__(self, neuron_types, source_indices, target_indices, weights,
                 ...
                 fwhm_cen=4.6, fwhm_sur=15.37, A_rel=0.04, tm1_coords=None, tm1_row_ids=None):
        super().__init__()
        self.device = device
        self.dt = dt
        self.tm1_tau_hp = tm1_tau_hp
        self.tm1_tau_lp = tm1_tau_lp
        self.tm1_sigma_c = (fwhm_cen / 4.6) * (2 / np.sqrt(3))/ (2*np.sqrt(2 * np.log(2)))  
        self.tm1_sigma_s = (fwhm_sur / 4.6) * (2 / np.sqrt(3))/ (2*np.sqrt(2 * np.log(2)))
        self.A_rel = A_rel
        if remove_reciprocal:
            self._source_indices, self._target_indices, self._weights = remove_lateral_connections(
    ...
    def forward(self, tm1_input, v_init=None, steps=None, return_history=False, steady_state_tol=1e-5):
        ...
        for step in range(steps):
            ...
            # stage 2: leaky rectification
            alpha = 0.26  
            rect_out = torch.where(hp_out >= 0, hp_out, alpha * hp_out)
            ...
            dv = (-v + synaptic_input + vrest.unsqueeze(0)) / tau.unsqueeze(0)
            dv[:, tm1_mask] = 0
